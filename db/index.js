...
export function migrarEsquema() {
  ...
  migracionPrincipiosDeViajero();
  migracionViajeroEnExcursiones();
  migracionViajeroEnElLienzo();
...
/**
 * LOS PRINCIPIOS DE VIAJERO EN EL REPARTO DE NOCHES.
 *
 * El codigo ya genera los repartos legales y la IA elige entre ellos. Lo que le
 * faltaba era el criterio con el que elegir: hasta ahora desempataba por peso y
 * poco mas, y eso deja fuera lo que de verdad nota quien viaja.
 *
 * Lo que se le anade no son reglas duras —esas viven en el codigo— sino la forma
 * de mirar: que un cambio de ciudad cuesta media jornada aunque el tren sean dos
 * horas, que un viaje que pasa un tercio de sus dias moviendose es un viaje
 * peor, y que una noche de mas vale distinto segun en que dia de la semana caiga.
 *
 * Con los dos ejemplos de casa —el reparto bueno de Polonia y el malo de cuatro
 * ciudades en seis noches—, que es lo que convierte un principio en algo
 * aplicable.
 */
function migracionPrincipiosDeViajero() {
  const CLAVE = '2026-09-principios-de-viajero';
  if (yaAplicada(CLAVE)) return false;
  ...
  const VIEJO = `LA REGLA DE LAS DOS NOCHES SE APLICA IGUAL PARA TODOS, O SE RAZONA:`;

  const NUEVO = `CÓMO PIENSA ALGUIEN QUE HA VIAJADO, Y ASÍ DESEMPATAS Y ASÍ LO ESCRIBES:

· CADA CAMBIO DE CIUDAD CUESTA MÁS QUE EL TRAYECTO. Hacer las maletas, el
  checkout, llegar a la estación, encontrar el hotel nuevo y deshacerlas otra
  vez. Cuenta cada salto como MEDIA JORNADA ÚTIL PERDIDA, no como los minutos
  del tren, y dilo así cuando compares dos repartos: "tres saltos se comen día y
  medio".

· UN VIAJE SANO NO DEDICA MÁS DE UN TERCIO DE SUS DÍAS A MOVERSE. Si un reparto
  lo supera, prefiere el que menos salte AUNQUE CUBRA UNA CIUDAD MENOS. Ver dos
  ciudades con calma es mejor viaje que ver cuatro por la ventanilla.

· AL DESEMPATAR ENTRE VARIANTES DE NOCHES, MIRA EL DÍA DE LA SEMANA. Tienes el
  calendario arriba: úsalo, no lo calcules de memoria. Una noche extra vale más
  donde el día que ganas NO sea lunes —museos cerrados— y donde quepa una
  excursión de día completo.

· SI LOS SALTOS INTERNOS SON EN AVIÓN O EN FERRY, DI LO QUE CUESTAN
  aproximadamente. Quien lee decide mejor sabiendo que la ruta bonita son 300 €
  más en traslados. Si no tienes el precio, dilo: "no sé lo que cuesta ese
  ferry".

ASÍ SE VE BIEN Y ASÍ SE VE MAL, con dos repartos de verdad de este proyecto:

  BIEN · 6 noches en Polonia, Cracovia pidiendo 3-4: Cracovia 4 + Varsovia 2, y
         Wrocław y Gdansk se quedan fuera. Dos bases, un solo salto, cero prisa.
  MAL  · 6 noches y cuatro ciudades: Wrocław 2, Poznań 1, Varsovia 2, Cracovia 1.
         Tres saltos, media semana deshaciendo maletas, y la ciudad más
         importante del viaje despachada con una tarde.

LA REGLA DE LAS DOS NOCHES SE APLICA IGUAL PARA TODOS, O SE RAZONA:`;

  // Y EL CALENDARIO, donde se leen las fechas.
  //
  // Sin esto el principio del lunes es una invitación a inventarse el día de la
  // semana: el prompt solo tenía «2026-09-21» y sacar de ahí que el día 3 es
  // miércoles es una cuenta. Se le da hecha, que es lo que se hace en esta casa
  // con todo lo que se puede calcular.
  const VIEJA_FECHA = `- Fechas: del {{FECHA_INICIO}} al {{FECHA_FIN}} ({{DIAS}} días, {{NOCHES}} noches)`;
  const NUEVA_FECHA = `${VIEJA_FECHA}
- Calendario: {{CALENDARIO}}`;

  const cambiar = (t) => {
    if (!t) return t;
    let x = t.includes(VIEJO) && !t.includes('CÓMO PIENSA ALGUIEN QUE HA VIAJADO')
      ? t.replace(VIEJO, NUEVO)
      : t;
    if (x.includes(VIEJA_FECHA) && !x.includes('{{CALENDARIO}}')) {
      x = x.replace(VIEJA_FECHA, NUEVA_FECHA);
    }
    return x;
  };
  ...
  marcarAplicada(CLAVE);
  console.log(
    `[bd] Migracion: principios de viajero ${nuevaFabrica !== fila.prompt_fabrica ? 'puestos' : 'NO puestos'}.`
  );
  return true;
}

/**
 * PRINCIPIOS DE VIAJERO AL PRESELECCIONAR EXCURSIONES.
 *
 * El bloque de «DIAS DISPONIBLES» ya impide preseleccionar mas excursiones
 * largas que dias hay, y eso es aritmetica: se queda donde esta y sigue mandando
 * sobre todo lo demas. Lo que se anade aqui es lo otro, lo que no se cuenta.
 *
 * Que una excursion de dia completo en una parada de dos noches se come el unico
 * dia entero de la ciudad —el fallo de Varsovia con Majdanek y el de Atenas con
 * Delfos, los dos en la misma semana—. Que una excursion a OTRA ciudad vale
 * menos que una del entorno, porque uno fue a ver la ciudad donde duerme. Y que
 * despues de un campo de concentracion el dia pide plan suave, que es de las
 * pocas cosas que un plan «correcto en horarios» puede hacer rematadamente mal.
 *
 * Va ANTES del bloque de dias disponibles a proposito: aquello es el tope duro y
 * tiene que seguir leyendose como la ultima palabra.
 */
function migracionViajeroEnExcursiones() {
  const CLAVE = '2026-09-viajero-en-excursiones';
  if (yaAplicada(CLAVE)) return false;
  ...
  const VIEJO = `DIAS DISPONIBLES, Y ESTO MANDA SOBRE TODO LO DEMAS:`;

  const NUEVO = `CÓMO ELIGE ALGUIEN QUE HA VIAJADO:

· UNA EXCURSIÓN DE DÍA COMPLETO EN UNA PARADA DE 2 NOCHES CASI NUNCA COMPENSA.
  Se come el único día entero que tiene la ciudad, y quien duerme dos noches en
  un sitio es para verlo. Mira arriba cuántas noches hay: con 2, preselecciona
  una larga SOLO si la ciudad tiene 3 o más días o si la excursión ES el motivo
  de la parada. Pasó de verdad dos veces en la misma semana: Majdanek se llevó el
  día de Varsovia y Delfos el de Atenas, y las dos ciudades se quedaron sin ver.

· LAS EXCURSIONES A OTRA CIUDAD VALEN MENOS QUE LAS DEL ENTORNO INMEDIATO.
  Auschwitz o Wieliczka desde Cracovia son la comarca; Majdanek desde Varsovia o
  Delfos desde Atenas son otro viaje dentro del viaje. El viajero fue a ver la
  ciudad donde duerme. No están prohibidas, pero tienen que ganarse el día con
  algo más que ser famosas.

· PREFIERE VARIEDAD DE TIPOS. Una cultural, una gastronómica y una de paisaje
  reparten mejor la semana que tres visitas guiadas de museo. Si dos candidatas
  empatan, gana la que no se parece a lo que ya hay elegido.

· DESPUÉS DE UNA EXCURSIÓN EMOCIONALMENTE DURA —campos de concentración,
  memoriales de guerra, museos del horror— el resto del día pide plan suave:
  paseo, cena tranquila, poco más. Ni encadenes dos duras el mismo día ni elijas
  dos si con una basta. Dilo en el "por_que" cuando elijas una de estas, para
  que quien reparta los días lo tenga delante.

ASÍ SE VE BIEN Y ASÍ SE VE MAL, con dos días de verdad de este proyecto:

  BIEN · Cracovia, día 2: Auschwitz por la mañana y después solo comer y una cena
         en Hawełka. Día duro con tarde amable.
  MAL  · Auschwitz por la mañana + Kazimierz entero con cementerio y sinagoga por
         la tarde + tour del vodka por la noche. Legal en horarios, inhumano en
         ritmo, y de un memorial no se sale con ganas de nada.

DIAS DISPONIBLES, Y ESTO MANDA SOBRE TODO LO DEMAS:`;

  const cambiar = (t) =>
    t && t.includes(VIEJO) && !t.includes('CÓMO ELIGE ALGUIEN QUE HA VIAJADO')
      ? t.replace(VIEJO, NUEVO)
      : t;
  ...
  marcarAplicada(CLAVE);
  console.log(
    `[bd] Migracion: principios de viajero en excursiones ${nuevaFabrica !== fila.prompt_fabrica ? 'puestos' : 'NO puestos'}.`
  );
  return true;
}

/**
 * PRINCIPIOS DE VIAJERO AL MONTAR CADA DIA.
 *
 * Las doce reglas que ya tenia el lienzo son de LEGALIDAD: horarios, bloques
 * fijos, margenes, no cruzar la ciudad dos veces. Y con todas cumplidas salio
 * esto en Varsovia: POLIN 4 h + Museo de la Insurgencia 2,5 h + taller de
 * pierogi 2 h + concierto de Chopin, el mismo dia. Ni un solape, ni un horario
 * roto, y una paliza.
 *
 * Lo que faltaba era el techo de CARGA —cuanto cabe en un dia sin que el dia
 * deje de ser un dia— y tres cosas mas que solo se notan viajando: que el plan
 * pise la zona del hotel, que tras un memorial la tarde va suave, y que una
 * comida se adelanta antes de dejarla pisada.
 *
 * El caso del hotel necesitaba ademas un dato que el prompt no tenia, y va
 * aparte en `orquestador-lienzo.js`: sin saber donde se duerme, «reserva un
 * atardecer para Oia» es pedirle al modelo que adivine donde esta Oia.
 */
function migracionViajeroEnElLienzo() {
  const CLAVE = '2026-09-viajero-en-el-lienzo';
  if (yaAplicada(CLAVE)) return false;
  ...
  const VIEJO = `LAS REGLAS:`;
  const NUEVO = `DONDE SE DUERME EN ESTA PARADA
{{HOTEL}}

LAS REGLAS:`;

  const COLA = `12. Los sitios marcados como [segundo nivel] entran DESPUÉS de los demás: son el
    relleno de los huecos que queden, no la primera opción.`;

  const MAS = `${COLA}

CÓMO MONTA UN DÍA ALGUIEN QUE HA VIAJADO:

· LA CARGA DE UN DÍA TIENE TECHO, y las reglas de arriba no lo ponen. Orientación:
  UN bloque grande (3-4 h) + DOS medianos + las comidas. Si te sale un día legal
  con más, quítale tú el de menor nivel antes de entregarlo y mándalo a "fuera"
  o a otro día. Pasó de verdad: POLIN 4 h + Museo de la Insurgencia 2,5 h +
  taller de pierogi 2 h + concierto de Chopin, todo el mismo día. Sin un solo
  solape y sin un horario roto. Una paliza.

· EL RECORRIDO DEL DÍA ES UNA LÍNEA, NO UN ZIGZAG. No basta con agrupar por
  zonas: ORDENA las visitas para que se avance en un sentido. Un día no cruza la
  ciudad dos veces.

· EL PLAN PISA LA ZONA DEL HOTEL. Al menos el paseo de la tarde o la cena, cerca
  de donde se duerme. Y si el hotel está en OTRA LOCALIDAD que el grueso del
  plan, esa localidad se gana un atardecer o una mañana: el viajero la eligió al
  reservar. En Santorini se durmió en Oia, el plan entero se montó en Fira y Oia
  acabó en "fuera" por falta de franja. Eso es no haber mirado dónde duerme.

· EL DÍA DE LLEGADA ES DE ATERRIZAJE: un paseo, una visita ligera y la cena. El
  DÍA DE SALIDA, nada que no esté a tiro de piedra del hotel.

· TRAS UNA EXCURSIÓN DURA DE MEMORIA HISTÓRICA —un campo, un memorial—, LA TARDE
  VA SUAVE. Aplícalo también cuando la excursión ya viene colocada y no la has
  elegido tú: de un sitio así no se sale con ganas de un museo más.

· LAS COMIDAS, A SU HORA: entre 13:00 y 15:00, y la cena no más tarde de las
  21:30 salvo que haya espectáculo. Antes de dejar una comida pisada por una
  visita, prueba a ADELANTARLA: casi siempre cabe media hora antes.

ASÍ SE VE BIEN, con un día de verdad de este proyecto:

  BIEN · Cracovia, día 1: se llega a las 13:30, la plaza, la basílica y cena en
         la misma plaza. Aterrizaje perfecto: poco, cerca y sin prisa.`;

  const cambiar = (t) => {
    if (!t) return t;
    if (t.includes('CÓMO MONTA UN DÍA ALGUIEN QUE HA VIAJADO')) return t;
    let x = t.includes(VIEJO) && !t.includes('{{HOTEL}}') ? t.replace(VIEJO, NUEVO) : t;
    if (x.includes(COLA)) x = x.replace(COLA, MAS);
    return x;
  };
  ...
  db.prepare(
    "UPDATE prompts_orquestador SET prompt_actual = ?, prompt_fabrica = ? WHERE fase = 'lienzo'"
  ).run(nuevoActual, nuevaFabrica);

  marcarAplicada(CLAVE);
  console.log(
    `[bd] Migracion: principios de viajero en el lienzo ${nuevaFabrica !== fila.prompt_fabrica ? 'puestos' : 'NO puestos'}.`
  );
  return true;
}
