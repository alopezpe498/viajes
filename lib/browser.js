 ...
 */
/**
 * EL SEMÁFORO DEL NAVEGADOR, Y POR QUÉ TIENE UNA SOLA PLAZA.
 *
 * ESTOS SCRAPERS COMPARTEN UN PERFIL. `launchPersistentContext` abre siempre el
 * mismo `browser-profile`, y un perfil de Chrome no se puede abrir dos veces: el
 * segundo ve el SingletonLock, delega en la instancia existente, se cierra, y
 * Playwright espera un navegador que ya no está. En el viaje de Asia eso se
 * llevó por delante casi todos los Booking, los datos duros de sitios y las
 * excursiones enteras.
 *
 * Y EL PERFIL NO ES OPCIONAL. Ahí viven las cookies y la sesión que hacen que
 * Booking no pida captcha y que Kayak no te enseñe la puerta; el propio código
 * de los proveedores lo dice, y por eso tampoco corren en headless. Con perfiles
 * nuevos por trabajador —la otra opción— cada uno empezaría frío y con la IP
 * repetida, que es la receta del bloqueo. Así que: UNA PLAZA, el scraping en
 * fila, y las llamadas de IA en paralelo, que es donde estaban los seis minutos
 * y medio que se querían recortar.
 *
 * `concurrencia_scraping` queda como parámetro y cada plaza usa SU perfil, de
 * modo que subirlo funciona de verdad el día que se quiera probar. De fábrica va
 * a 1, que es lo único que estos sitios aguantan hoy.
 *
 * LA PLAZA SE COGE AL ABRIR Y SE SUELTA AL CERRAR, no alrededor del arranque.
 * Ese fue el fallo de la primera versión: envolvía `arrancarNavegador`, soltaba
 * en cuanto la ventana abría y la sesión —que dura minutos— corría sin
 * protección. El segundo Chrome llegaba mientras el primero seguía dentro.
 */
const plazas = semaforo(() => parametro('concurrencia_scraping', 1));

/** Qué plaza tiene cada contexto abierto, para poder soltarla al cerrar. */
const plazaDe = new Map();

/**
 * CUÁNTO SE AGUANTA UNA PLAZA COMO MUCHO.
 *
 * Los seis proveedores cierran en un `finally`, así que la plaza siempre se
 * suelta. Pero si alguno se colgara antes de llegar ahí, el semáforo se quedaría
 * cerrado y el viaje entero se quedaría sin scraping en silencio. A los quince
 * minutos se suelta sola y se dice: mejor un navegador huérfano que un montaje
 * parado para siempre.
 */
const TOPE_DE_PLAZA_MS = 15 * 60 * 1000;
...
export async function abrirNavegador(opciones = {}) {
  const { plaza, soltar } = await plazas.coger();

  // Si la plaza se queda tomada por un proveedor que no cierra, se suelta sola.
  const alarma = setTimeout(() => {
    console.warn(
      `[browser] La plaza ${plaza} lleva ${TOPE_DE_PLAZA_MS / 60000} min tomada: la suelto a la fuerza.`
    );
    soltar();
  }, TOPE_DE_PLAZA_MS);
  ...
  //
  // Con una sola plaza esto no debería saltar nunca, pero un Chrome zombi de una
  // ejecución anterior deja el SingletonLock puesto y el arranque falla por algo
  // que se arregla solo en unos segundos. Darlo por perdido a la primera es lo
  // que convirtió un tropiezo en «0 excursiones preseleccionadas».
  const veces = Math.max(0, Math.min(5, parametro('reintentos_navegador', 2)));

  for (let intento = 0; ; intento += 1) {
    try {
      const abierto = await arrancarNavegador({ ...opciones, plaza });
      ...
      console.warn(
        `[browser] El perfil estaba en uso (${err.message.slice(0, 80)}). ` +
          `Reintento ${intento + 1} de ${veces} en ${espera / 1000}s.`
      );
      await dormir(espera);
    }
  }
...
async function arrancarNavegador({ headless = false, de = null, plaza = 0 } = {}) {
  // CADA PLAZA, SU PERFIL. La 0 es el de siempre —el que tiene las sesiones
  // calientes— y las demás son copias vírgenes. Mientras haya una sola plaza,
  // esto es exactamente el comportamiento de antes.
  const perfil = plaza ? `${DIR_PERFIL}-${plaza}` : DIR_PERFIL;
