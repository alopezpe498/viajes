...
export async function consultarJSON(
  ...
  for (let intento = 1; intento <= 2; intento++) {
    ...
    try {
      ...
      const reparado = limpiar(ultimoTexto).replace(/,(\s*[}\]])/g, '$1');
      try {
        const r = JSON.parse(reparado);
        console.warn(`[ia] La respuesta de ${paso} traía una coma de más. La he quitado y parsea.`);
        return r;
      } catch {
        /* no era eso: sigue el camino de siempre */
      }
