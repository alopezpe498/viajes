...
export async function consultarJSON(
  prompt,
  { maxTokens = 8000, paso = 'la consulta', conWeb = false, modelo = null } = {}
) {
  ...
  for (let intento = 1; intento <= 2; intento++) {
    ...
    try {
      ultimoTexto = await pedirTexto({ clave, mensaje, maxTokens, paso, web, modelo });
    } catch (err) {
...
async function unaLlamada({ clave, mensaje, maxTokens, paso, web = false, modelo: opcionesDeModelo = null }) {
