...
`);
  ...
  try {
    const r = await consultarJSON(prompt, {
      maxTokens: 300,
      paso: 'resolver el código IATA',
      // UN CÓDIGO DE TRES LETRAS NO SE PIENSA, SE SABE.
      //
      // Vive en la fase `ciudades_y_noches`, que está en criterio porque elegir
      // la puerta y la ruta sí se piensa. Heredando, cada puerta que se prueba
      // —hasta cinco por viaje— pagaba el modelo caro para traducir «Cracovia»
      // a «KRK». La respuesta es correcta o no lo es; no hay nada que razonar, y
      // ya trae su propia red: si `seguro` viene false, se descarta.
      modelo: 'rapido',
    });
