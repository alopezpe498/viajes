...
// La matriz es de la MISMA API que las rutas —Routes—, no la Distance Matrix
// clasica, que Google tiene en legacy desde 2025 y contesta otro formato.
...
/**
 * EL ESPEJO DEL ANTERIOR: ¿ME HAN DEVUELTO UN SITIO, O LA CIUDAD ENTERA?
 *
 * `pareceUnaCiudad` pregunta si un resultado sirve como punto de una ciudad, y
 * por eso acepta `locality` de buena gana. Esta pregunta lo contrario, y es la
 * que faltaba: cuando se busca un teatro, un museo o un castillo, que Google
 * conteste con la ciudad entera NO es una respuesta aproximada, es otra cosa.
 *
 * Así se juntaron en un solo pin el Teatro de Epidauro (a 26 km), el yacimiento
 * de Micenas (a 20 km), la mezquita de Voivode y la isla de Bourtzi: los cuatro
 * volvieron con `tipos: [locality, political]` y las coordenadas del centro de
 * Nauplia, y nadie miró los tipos.
 *
 * LO QUE NO SE RECHAZA, y es deliberado:
 *
 *   · `neighborhood` y `sublocality`. Plaka, Psyrri, Kazimierz: un barrio es un
 *     sitio legítimo de este catálogo y su centro significa algo. El centro de
 *     una ciudad, para un monumento, no.
 *   · Lo que no trae tipos. Sin dato no se acusa a nadie, igual que arriba.
 *
 * La señal es QUÉ CLASE DE COSA dice Google que ha encontrado, no dónde cayó.
 * Medir la distancia al centro de la ciudad no vale: de los 99 sitios situados
 * del catálogo, 43 están a menos de un kilómetro del centro de su ciudad y casi
 * todos son correctos —el Partenón, la plaza del Mercado de Cracovia—. La
 * distancia acusa a los inocentes; el tipo, no.
 */
export function pareceUnSitio(hallado) {
  if (!hallado) return false;
  const tipos = hallado.tipos ?? [];
  if (!tipos.length) return true; // sin dato no se acusa a nadie

  // Que aparezca cualquiera de estos ya dice que es un sitio de verdad, aunque
  // venga acompañado de otra cosa.
  const DE_UN_SITIO = [
    'establishment', 'point_of_interest', 'premise', 'subpremise',
    'street_address', 'route', 'intersection', 'natural_feature',
    'neighborhood', 'sublocality', 'park', 'airport', 'transit_station',
  ];
  if (tipos.some((t) => DE_UN_SITIO.includes(t))) return true;

  // Y si no hay ninguno, que no quede más que territorio es el caso malo.
  //
  // `postal_code` estuvo aquí y se ha caído de la lista con motivo: la
  // dirección guardada del Partenón es «Atenas 105 58, Grecia» y la del Ágora
  // Antigua «Atenas 105 55», que son códigos postales y están bien. Un código
  // postal abarca unas manzanas, no un término municipal, así que su centro
  // dice algo. Rechazarlo dejaba sin situar a tres sitios correctos.
  const DE_TODO_EL_TERMINO = [
    'locality', 'postal_town', 'colloquial_area', 'country',
    'archipelago', 'continent',
  ];
  return !tipos.some(
    (t) => DE_TODO_EL_TERMINO.includes(t) || t.startsWith('administrative_area_level_')
  );
}
...
/**
 * LO MISMO QUE `rutasConGoogle`, PERO PARA VARIOS PARES DE UNA VEZ.
 *
 * `rutasConGoogle` gasta una llamada por modo y por par. Un día con ocho puntos
 * y tres modos son veintiuna llamadas; con la matriz, tres. La API es la MISMA
 * —`computeRouteMatrix` de la Routes API, la que ya se usa en todo el fichero—,
 * no la Distance Matrix clásica, que Google tiene en modo legacy desde 2025 y
 * contesta con otro formato de error que habría que tratar aparte para nada.
 *
 * UNA LLAMADA POR MODO, no una por par: la matriz cruza N orígenes con M
 * destinos, pero el `travelMode` es de la petición entera.
 *
 * Y NO ES UN PRODUCTO CARTESIANO CUALQUIERA: aquí se piden PAREJAS concretas —el
 * hotel con cada sitio del día, no todos contra todos—, así que se manda cada
 * par como su propio origen y su propio destino y se lee la diagonal. Google
 * devuelve `originIndex` y `destinationIndex`, y solo interesan los que
 * coinciden. Pedir el cuadrado entero costaría N² celdas para usar N.
 *
 * Devuelve un array del MISMO tamaño y EN EL MISMO ORDEN que `pares`, con un
 * array de resultados en cada hueco, o `null` si Google no está disponible. El
 * orden importa: quien llama empareja por índice.
 */
export async function matrizConGoogle(pares, modos = Object.keys(MODOS)) {
  if (!googleDisponible()) return null;
  if (!Array.isArray(pares) || !pares.length) return [];
  if (pares.some(([a, b]) => !punto(a) || !punto(b))) return null;

  const salidas = pares.map(() => []);
  let algunaRespondio = false;
  ...
  for (const modo of modos) {
    ...
    const { cuerpo, fallo } = await pedir(
      URL_MATRIZ,
      {
        ...
        headers: {
          'Content-Type': 'application/json',
          'X-Goog-Api-Key': process.env.GOOGLE_MAPS_SERVER_KEY,
          // `condition` hace falta: una celda sin ruta viene con `ROUTE_NOT_FOUND`
          // y sin duración, y sin pedirla no habría forma de distinguirla de un
          // cero. Los índices también, o no se sabe de qué par es cada celda.
          'X-Goog-FieldMask':
            'originIndex,destinationIndex,duration,distanceMeters,condition',
        },
        body: JSON.stringify({
          origins: pares.map(([a]) => comoParada(a)),
          destinations: pares.map(([, b]) => comoParada(b)),
          travelMode: def.google,
          languageCode: 'es',
          units: 'METRIC',
          ...(def.google === 'DRIVE' ? { routingPreference: 'TRAFFIC_AWARE' } : {}),
    ...
    if (fallo !== undefined) {
      // Si el primer modo falla por la clave, los otros dos fallarán igual.
      if (!googleDisponible()) return null;
      continue;
    }
    ...
    // La respuesta es un array de celdas, no una tabla: viene desordenada y hay
    // que colocarla por sus índices.
    for (const celda of Array.isArray(cuerpo) ? cuerpo : []) {
      const i = celda.originIndex;
      const j = celda.destinationIndex;
      // Solo la diagonal: el par i es (origen i, destino i). Las demás celdas
      // son cruces que nadie ha pedido.
      if (i !== j || !salidas[i]) continue;
      if (celda.condition && celda.condition !== 'ROUTE_EXISTS') continue;
      if (celda.duration == null) continue;

      salidas[i].push({
        modo,
        minutos: Math.max(1, Math.round(Number(String(celda.duration).replace('s', '')) / 60)),
        km: celda.distanceMeters != null ? Number(celda.distanceMeters) / 1000 : null,
        fuente: 'google',
      });
    }
  }
  ...
  apuntarAcierto();
  const celdas = salidas.reduce((n, s) => n + s.length, 0);
  console.log(
    `[google] matriz OK: ${pares.length} par(es) x ${modos.length} modo(s) ` +
      `en ${modos.length} llamada(s) — ${celdas} resultado(s).`
  );
  return salidas;
}
...
export async function situarLugarConGoogle(nombre, ciudad = null) {
  ...
  const { cuerpo, fallo } = await pedir(
    URL_PLACES_BUSCAR,
    {
      ...
      headers: {
        'Content-Type': 'application/json',
        'X-Goog-Api-Key': process.env.GOOGLE_MAPS_SERVER_KEY,
        // `types` es lo que distingue un SITIO de una ZONA: un
        // `tourist_attraction` es un punto al que se va, un `locality` o un
        // `neighborhood` es un área entera y su coordenada es un centroide.
        // Quien decide si un resultado vale necesita saberlo.
        'X-Goog-FieldMask':
  ...
  return {
    ...
    tipos: Array.isArray(sitio.types) ? sitio.types : [],
  };
}
