 ...
 * que quien llama se vaya al plan B (Nominatim y OSRM, que no piden clave). En
 * local se desarrolla contra el plan B; en el servidor entra Google y se nota
 * porque aparece el transporte público, que OSRM no sabe dar.
 *
...
function apuntarFallo(donde, motivo) {
  ...
  if (noesSeguidos >= NOES_SEGUIDOS_PARA_RENDIRSE && !apagadaPorFallos) {
    ...
    console.warn(
      `[google] ${noesSeguidos} fallos seguidos: dejo de preguntar y tiro de Nominatim/OSRM. ` +
        'Es lo normal en local: la clave está restringida a la IP del servidor.'
    );
  }
  return null;
}
 ...
 * pantalla. El transporte público es el que justifica pagar por Google: OSRM no
...
export async function buscarSitiosConGoogle(
  ...
  const cuerpo = {
    ...
    // Solo comida: sin esto "cerca del hotel" devuelve hoteles.
