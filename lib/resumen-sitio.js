...
/**
 * UN PRECIO EN CUALQUIER MONEDA, solo para saber SI lo hay y DÓNDE.
 *
 * `IMPORTE` sabe de euros y nada más, y eso dejaba ciega la regla del «gratis va
 * delante» en media Polonia: «General: 32 PLN. Reducida: 28 PLN. Gratis los
 * lunes» no tenía para el código ni un solo importe, así que el «gratis» del
 * final pasaba por titular y la Fábrica de Schindler salía «Gratis» costando 32
 * eslotis. Seis fichas del viaje a Polonia estaban así.
 *
 * Esta NO se usa para pintar: el valor sigue saliendo por el camino de siempre,
 * que para lo que no son euros acaba recortando el propio texto —«General: 32
 * PLN»—, que es lo que ya hacían las fichas polacas sin «gratis» detrás.
 */
const IMPORTE_EN_LO_QUE_SEA =
  /(\d{1,4}(?:[.,]\d{1,2})?)\s*(?:€|\$|£|PLN|zł|CZK|Kč|HUF|Ft|SEK|NOK|DKK|CHF|USD|EUR|GBP)|[€$£]\s*\d/i;

/** Cómo se dice que algo no cuesta nada. Sin `g`: interesa DÓNDE está. */
const GRATIS = /\b(gratis|gratuit|entrada libre|sin coste|acceso libre)/i;

/**
 * QUIÉN PAGA MENOS: las tarifas que no son la de un adulto cualquiera.
 *
 * Se busca en el texto ORIGINAL, con sus paréntesis, porque ahí es donde la
 * mitad de las veces está la palabra: «15 € (General) / 8 € (Reducida)». Si se
 * mirara el texto ya limpio, esa ficha no tendría ninguna marca y se colaría.
 */
const CONCESION = /\b(reducid|estudiant|jubilad|mayores de|menores de|infantil|niñ[oa]s|senior|desemplead|discapacid|carné joven)/i;
...
export function resumirPrecio(texto) {
  ...
  // GRATIS SOLO SI VA DELANTE. La comprobación miraba si la palabra estaba, en
  // cualquier parte, y devolvía ahí mismo sin llegar a ver ni un importe. Con
  // «Entrada general: 10 €, Tarifa reducida: 5 €, Menores de 6 años: Gratis» el
  // Estadio Panatenaico salía «Gratis» en la tarjeta teniendo sus propios datos
  // el precio debajo.
  //
  // El comentario de aquí arriba ya decía la regla buena —«lo que cuesta entrar
  // va siempre delante»— y el código no comprobaba el delante. Ahora sí: la
  // palabra manda si abre el texto, y es un tramo más si viene detrás de un
  // precio.
  const gratis = GRATIS.exec(crudo);
  const hallazgos = [...crudo.matchAll(IMPORTE)];

  // Para decidir si «gratis» es el titular vale un precio en CUALQUIER moneda,
  // no solo en euros: el titular de una ficha de Cracovia se juega en eslotis.
  if (gratis) {
    const enLoQueSea = IMPORTE_EN_LO_QUE_SEA.exec(crudo);
    const hayPrecioDelante = enLoQueSea && enLoQueSea.index < gratis.index;
    if (!hayPrecioDelante) return 'Gratis';
  }

  const importes = hallazgos.map((m) => m[1] ?? m[2]);
  if (!importes.length) return recorte(crudo, 22);

  const enNumero = (s) => Number(String(s).replace(',', '.'));
  const dice = /\bdesde\b|\ba partir de\b|\bentre\b/i.test(crudo);

  // LA TARIFA GENERAL MANDA SOBRE LA REDUCIDA.
  //
  // Un descuento de estudiante no es «lo que cuesta entrar»: es lo que cuesta si
  // resulta que tienes derecho a él. Coger el importe más bajo dejaba el Estadio
  // Panatenaico en «desde 5 €» siendo 10 € la entrada normal.
  //
  // Y la aplicación ya lo hacía a medias, sin querer: «20 € (Reducida: 10 €)»
  // salía «20 €» —porque el paréntesis se tira antes— y «15 € (General) / 8 €
  // (Reducida)» salía «desde 8 €». La misma situación con dos respuestas
  // distintas, según dónde cayeran los paréntesis.
  //
  // Se coge EL PRIMERO, no el mayor: estos textos ponen siempre la general
  // delante («Entrada general: 10 €, Tarifa reducida: 5 €»), y el mayor se
  // equivocaría con un combinado que valga más que la entrada suelta.
  if (CONCESION.test(texto)) {
    const bonito = `${String(importes[0]).replace('.', ',')} €`;
    // Aquí no basta con que haya varios importes para decir «desde»: los otros
    // SON las tarifas reducidas. Solo si el texto lo dice con todas las letras.
    return dice ? `desde ${bonito}` : bonito;
  }
  ...
  const hayTramo = importes.length > 1 || dice;
